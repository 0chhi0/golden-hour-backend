...
// Vollständige Welt-Matrix mit ca. 195 ISO-Codes und deren zentralen Längengraden
const worldData = [
...
/**
 * Filtert Länder, in denen potenziell Golden Hour herrscht.
 * Nutzt SunCalc für die exakte Altitude-Berechnung.
 */
function getGoldenHourCountries() {
    const now = new Date();
    return worldData.filter(c => {
        // Wir nehmen den Äquator (lat: 0) als Referenz für den Längengrad des Landes
        const sunPos = SunCalc.getPosition(now, 0, c.lon);
        const altitude = sunPos.altitude * 180 / Math.PI;
        
        // Puffer: Wir nehmen alles zwischen -12° und +12°, 
        // damit das Frontend (-6° bis +6°) sicher Treffer findet.
        return (altitude >= -12 && altitude <= 12);
    });
}
...
app.get('/api/webcams', async (req, res) => {
    try {
        ...
        console.log(`🌞 Synchronisierter Scan: ${targetCountries.length} Länder im SunCalc-Fokus.`);

        // Parallelisierte Abfrage aller Fokus-Länder
        const fetchPromises = targetCountries.map(c => 
            fetch(`https://api.windy.com/webcams/api/v3/webcams?limit=50&country=${c.id}&include=location,player`, {
        ...
        results.forEach(data => {
            if (data.webcams) {
                // Filtert nur funktionale Streams/Videos
        ...
        console.log(`📊 Strategisches Ergebnis: ${uniqueWebcams.length} Webcams an das Frontend gesendet.`);
...
app.listen(PORT, () => console.log(`🚀 Golden-Hour-Backend (v3) läuft auf Port ${PORT}`));
