...
// Vollständige Welt-Matrix (Längengrade)
const worldData = [
...
app.get('/api/webcams', async (req, res) => {
    try {
        ...
        // Solar Noon Berechnung (Gleicher Algorithmus wie im Frontend)
        let solarNoonLon = 180 - (utcHour * 15);
        if (solarNoonLon > 180) solarNoonLon -= 360;
        if (solarNoonLon < -180) solarNoonLon += 360;

        // Breiterer Filter (30° bis 150°), um sicherzustellen, dass die Frontend-Boxen Treffer landen
        const targetCountries = worldData.filter(c => {
            const diff = Math.min(Math.abs(c.lon - solarNoonLon), 360 - Math.abs(c.lon - solarNoonLon));
            return (diff > 30 && diff < 150); 
        });

        console.log(`🌞 Dynamischer Scan: ${targetCountries.length} Länder im erweiterten Licht-Fokus.`);

        // Begrenzung auf 45 Anfragen, um API-Sperren zu vermeiden
        const fetchPromises = targetCountries.slice(0, 45).map(c => 
            fetch(`https://api.windy.com/webcams/api/v3/webcams?limit=30&country=${c.id}&include=location,player`, {
        ...
        results.forEach(data => {
            if (data.webcams) {
                // Smart-Filter für Video-Qualität (erfasst auch Missouri 1731969577)
        ...
        console.log(`📊 Ergebnis: ${uniqueWebcams.length} Webcams geliefert.`);
...
app.listen(PORT, () => console.log(`🚀 Smart-Dynamic-Backend aktiv.`));
