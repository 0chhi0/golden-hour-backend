...
// Die vollständige Länderliste für den Scan
const worldData = [
...
app.get('/api/webcams', async (req, res) => {
    try {
        const now = new Date();
        // 1. Länder filtern, die in der erweiterten Dämmerungszone liegen
        ...
        console.log(`📡 Stapel-Scan: Frage ${targetCountries.length} Länder einzeln ab...`);

        // 2. Stapel-Logik: Für jedes Land eine eigene Abfrage (max 50)
        const countryRequests = targetCountries.map(async (country) => {
            try {
        ...
        // 3. Auf alle Länder warten und Ergebnisse stapeln
        const allResults = await Promise.all(countryRequests);
        const combinedWebcams = allResults.flat();

        // 4. Dubletten entfernen
        const uniqueWebcams = Array.from(new Map(combinedWebcams.map(w => [w.webcamId, w])).values());

        console.log(`✅ Scan beendet. Stapelgröße: ${uniqueWebcams.length} Webcams.`);
...
app.listen(PORT, () => console.log(`🚀 Stapel-Backend v4 aktiv.`));
